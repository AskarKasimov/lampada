...
- Для PR используйте только **Squash and merge**. Один PR должен быть одной
  законченной задачей; после merge его ветку удаляют.
...
1. Создайте от `master` ветку `release/<версия>`.
...
3. Влейте release-PR в `master` после CI и review.
4. Создайте и отправьте тег `vX.Y.Z` на squash-коммите в `master`.
