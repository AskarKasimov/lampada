...
одна карточка контента на весь экран. Порядок внутри дня: цитата → вопрос →
совет → основы → чтение. Контент — дневные фиды Азбуки веры.
...
«Вопрос дня» цепляется не за контейнер (у него общий `class="widget"`, таких
на странице несколько), а за уникальный класс ссылки `a.az-qod-link`. На
странице лежит только текст вопроса — ответ живёт на отдельной странице по
