...
Бюджет загрузки — 10 секунд на весь цикл попыток. Через 3 секунды ожидания
под брендингом появляется спиннер (`BrandLoadingView`): без него неподвижный
логотип читается как зависшее приложение. Тот же виджет — в загрузочном
состоянии Home, куда возвращает «Повторить».
